...
class UnlabeledImageFolder(Dataset):
    def __init__(self, root, transform):
        ...
        self.images = [f for f in os.listdir(self.root) if not f.startswith(".")]
...
def my_app(cfg: DictConfig) -> None:
    ...
    model = LitUnsupervisedSegmenter.load_from_checkpoint(cfg.model_path, map_location=torch.device('cpu'))
    ...
    for i, (img, name) in enumerate(tqdm(loader)):
        with torch.no_grad():
            ...
            features = np.array(code.permute(1,2,0).reshape(-1, 90))
            meta_points = clustering.create_meta_points(features, 7)
            collapsed_meta_points = clustering.collapse_meta_points(meta_points, features, 12)
            labels_meta_points = clustering.get_labels_meta_points2(collapsed_meta_points, features, 0.1)
            # Find number of unique labels
            num_labels = np.max(labels_meta_points) + 1
            identity = np.eye(num_labels.astype("int8"))
            
            collapsed_probs = identity[labels_meta_points]
            ...
            for j in range(img.shape[0]):
                ...
                Image.fromarray(collapsing_crf.astype(np.uint8)).save(join(result_dir, "collapsing", new_name))
