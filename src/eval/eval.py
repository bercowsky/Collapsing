...
The script runs inference on the images of the ADE20K validation set and
...
def png_path_name(model_name, image_name):
    ...
    return os.path.join(COLLAPSING_PATH, model_name + image_name.split('.')[0] + '.png')
...
def inference(model, dataloader, config=None):
    ...
    kmeans = KMeans(n_clusters=min(len(centers), target_clusters), n_init='auto', random_state=0).fit(centers)
    ...
    confusion_matrix_collapsing = np.zeros((n_clusters, target_clusters+1))
    confusion_matrix_kmeans = np.zeros((target_clusters+1, target_clusters+1))
    for batch_idx, samples in tqdm(enumerate(dataloader), total=config.num_images):
        if batch_idx == config.num_images:
            break
